...
class Database:
    ...
    def migrate_from_json(self, data_dir: str):
        """Import existing JSON tenant data into SQLite."""
        from .storage import StorageManager
        sm = StorageManager(data_dir)

        for tenant_name in sm.list_tenants():
            store = sm.get_tenant_store(tenant_name)
            config = store.load_config()
            if config:
                try:
                    self.create_tenant(tenant_name, config)
                except sqlite3.IntegrityError:
                    pass  # Tenant already exists

            if sm.get_active_tenant() == tenant_name:
            ...
            for action in actions:
                d = action.to_dict()
                try:
                    self.create_action(tenant_name, d)
                except sqlite3.IntegrityError:
                    pass

            for entry in store.load_import_history():
                with self._conn() as conn:
                    conn.execute(
                        """INSERT INTO import_history (tenant_name, timestamp, source_tool,
                           file_path, action_count, new_actions, updated_actions)
                           VALUES (?, ?, ?, ?, ?, ?, ?)""",
                        (tenant_name, entry.get("timestamp", ""),
                         entry.get("source_tool", ""),
                         entry.get("file_path", ""),
                         entry.get("action_count", 0),
                         entry.get("new_actions", 0),
                         entry.get("updated_actions", 0)),
                    )

        return {"migrated": True, "tenants": sm.list_tenants()}
